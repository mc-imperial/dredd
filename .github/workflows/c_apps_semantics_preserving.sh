...
pushd curl
  ...
  "${DREDD_EXECUTABLE}" --semantics-preserving-coverage-instrumentation --mutation-info-file temp.json -p "build/compile_commands.json" "${FILES[@]}"
  pushd build
    ninja
    # TODO: run some tests
  popd
...
pushd zstd
  ...
  "${DREDD_EXECUTABLE}" --semantics-preserving-coverage-instrumentation --mutation-info-file temp.json -p "temp/compile_commands.json" "${FILES[@]}"
  # Build mutated zstd
  make clean
  CFLAGS=-O0 make zstd-release
