...
```
/path/to/dredd -p build/compile_commands.json <path-to-file> --mutation-info-file temp.json
``` 
For example, running:

```
/path/to/dredd -p build/compile_commands.json math/src/exp.cc --mutation-info-file temp.json
```

from the `examples/math` directory will apply mutants to the file `exp.cc`. 
...
Now, from the `examples/math` directory, run:
```
./mutate.sh
```
...
```
DREDD_ENABLED_MUTATION=<mutation_id> ./build/mathtest/mathtest
```
where `<mutation_id>` is the id of the mutation in the modified library generated by Dredd. For example, running:
...
To clean up the `examples/math` directory, run the following:
```
rm temp.json
rm -rf build
git checkout HEAD .
